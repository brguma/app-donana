...
const App = () => {
  ...
  if (currentScreen === 'pedidos') {
    ...
    return (
      <div className="min-h-screen bg-pink-50 p-4">
        <div className="max-w-md mx-auto">
          <h2 className="text-2xl font-bold text-pink-800 mb-4">Pedidos Confirmados</h2>
          
          {/* Dica de edição */}
          <div className="bg-blue-100 border border-blue-400 text-blue-700 px-3 py-2 rounded mb-4 text-sm">
            💡 <strong>Campos editáveis:</strong> Nome do cliente, data de entrega, valor do sinal, tema da festa e quantidade dos itens. 
            <br />
            <span className="text-xs">📊 O total será recalculado automaticamente ao editar quantidades.</span>
          </div>

          {/* Mensagem de sincronização */}
          {syncMessage && (
            <div className="bg-green-200 border border-green-400 text-green-800 px-3 py-2 rounded mb-4 text-center text-sm">
              {syncMessage}
            </div>
          )}
              ...
              <div key={`pedido-${idx}`} className="bg-white p-4 rounded-lg shadow mb-4">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    {editingCliente === pedido.id ? (
                      ...
                      <div 
                        className="font-bold text-lg cursor-pointer hover:bg-gray-50 p-1 rounded flex items-center gap-2 transition-colors"
                    ...
                    {editingDataEntrega === pedido.id ? (
                      <div className="mt-1">
                        <input
                          type="date"
                          defaultValue={(() => {
                            try {
                              return new Date(pedido.dataEntrega).toISOString().slice(0, 10);
                            } catch {
                              return '';
                            }
                          })()}
                          onKeyPress={(e) => {
                            if (e.key === 'Enter') savePedidoDataEdit(pedido.id, e.target.value);
                          }}
                          onBlur={(e) => savePedidoDataEdit(pedido.id, e.target.value)}
                      ...
                      <div 
                        className="text-sm font-medium text-blue-600 cursor-pointer hover:bg-blue-50 p-1 rounded flex items-center gap-2 transition-colors"
                        onClick={() => setEditingDataEntrega(pedido.id)}
                      >
                    ...
                    {editingTema === pedido.id ? (
                      ...
                      <div 
                        className="text-sm text-purple-600 font-medium cursor-pointer hover:bg-purple-50 p-1 rounded flex items-center gap-2 mt-1 transition-colors"
                    ...
                    {pedido.sinal !== undefined && (
                      editingSinal === pedido.id ? (
                        <div className="mt-1">
                          <input
                            type="number"
                            step="0.01"
                            defaultValue={pedido.sinal || 0}
                            onKeyPress={(e) => {
                              if (e.key === 'Enter') savePedidoSinalEdit(pedido.id, e.target.value);
                            }}
                            onBlur={(e) => savePedidoSinalEdit(pedido.id, e.target.value)}
                        ...
                        <div 
                          className="text-sm text-gray-600 cursor-pointer hover:bg-gray-50 p-1 rounded flex items-center gap-2 transition-colors"
                          onClick={() => setEditingSinal(pedido.id)}
                        >
                          Sinal: {formatCurrency(pedido.sinal || 0)}
                          <Edit3 size={12} className="text-gray-400" />
                        </div>
                      )
                    )}
                  ...
                  <div className="text-right">
                    <div className="text-lg font-bold text-green-600">{formatCurrency(pedido.total || 0)}</div>
                    {pedido.sinal !== undefined && pedido.sinal >= 0 && (
                      <div className="text-sm font-medium text-orange-600">
                        Restante: {formatCurrency(Math.max(0, (pedido.total || 0) - (pedido.sinal || 0)))}
                      </div>
                    )}
                  </div>
                </div>
                <div className="border-t pt-2 mt-2">
                  {pedido.itens && Array.isArray(pedido.itens) && pedido.itens.map((item, itemIdx) => {
                    ...
                    return (
                      <div key={`item-${itemIdx}`} className="flex justify-between text-sm mb-1 items-center">
                        <div className="flex-1">
                          <span>{item.produto?.nome || 'Produto'}</span>
                          <span className="mx-1">x</span>
                          {editingItemQuantidade === `${pedido.id}-${itemIdx}` ? (
                            <input
                              type="number"
                              min="1"
                              defaultValue={itemQuantidade}
                              onKeyPress={(e) => {
                                if (e.key === 'Enter') savePedidoItemQuantidadeEdit(pedido.id, itemIdx, e.target.value);
                              }}
                              onBlur={(e) => savePedidoItemQuantidadeEdit(pedido.id, itemIdx, e.target.value)}
                              className="w-16 p-1 text-xs border border-gray-300 rounded text-center"
                              autoFocus
                            />
                          ) : (
                            <span 
                              className="cursor-pointer hover:bg-blue-50 px-1 rounded inline-flex items-center gap-1 transition-colors"
                              onClick={() => setEditingItemQuantidade(`${pedido.id}-${itemIdx}`)}
